...
int main(int argc, char *argv[])
{
     ...
     */
    char config[50] = "./Conf/params_1.txt";
    ...
    if (readConfigParameters(config) != FALSE){
		...
		else
		{
		    if (sigaction(SIGINT, &act, NULL) == -1)
		        unsafeErrorPrint("[MASTER]: failed to interrupt signal disposition. Error:", __LINE__);
		    else
		    {
...
void endOfSimulation(int sig)
{
    ...
    if (getpid() != masterPid)
        safeErrorPrint("[MASTER]: An error occurred while terminating the simulation: ", __LINE__);
    else
    {
