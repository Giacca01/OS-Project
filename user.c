                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                /* 
                                    if we arrive here and errAfterComputing is true, 
                                    an error occurred only releasing wrPartSem 
                                */
            ...
            tList->currTrans.receiver == t->receiver)
        {
            ...
            else
            {
                ...
                if (tList->nextTrans == NULL)
                {
                     ...
                     */
                    headPointer = NULL;
                    /* if we are still in the loop, we are sure that tList will not be NULL */
                    free(tList); 
                }
                else
                {
 ...
 *  -1 -> end of execution and deallocation (called from error);
 * SIGUSR1 -> end of execution and deallocation (called by signal from master)
 */
void endOfExecution(int sig)
{
    ...
    if (sig == SIGUSR1)
    {
         ...
         * termination EXIT_SUCCESS; otherwise, the execution ends because of insufficient budget.
         * It is also not necessary to signal termination to the master 
         * because it was the master who asked the node to terminate
         */
    }
    else
    {
...
void transactionGeneration(int sig)
{
    ...
    if (bilancio > 2)
    {
        /* I managed to send the transaction 
            I reset the counter of consecutive failures 
        */
        num_failure = 0; 
        ...
        else
        {
            ...
            /* calculation of the budget between 2 and the budget (so it only does it whole) */
            new_trans.amountSend = (randTime.tv_nsec % bilancio) + 2;
            /* if we suppose that SO_REWARD is a value (percentage) expressed between 1 and 100 */
            new_trans.reward = (new_trans.amountSend / 100) * SO_REWARD; 
            if (new_trans.reward < 1)
                new_trans.reward = 1;

            /* update amount sent to receiver removing node's reward */
            new_trans.amountSend -= new_trans.reward;

            /* get timestamp for transaction */
            clock_gettime(CLOCK_REALTIME, &new_trans.timestamp); 
...
void userFailure()
{
    ...
    /* I increase the consecutive number of times I can't send a transaction */
    num_failure++; 
